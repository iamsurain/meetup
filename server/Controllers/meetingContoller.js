...
export const createMeeting = async (req, res) => {
  try {
    ...
    // check meetings limit per calender month
    if (userPlan === "free") {
      ...
      if (monthlyCount >= 30) {
        return res.json({
          error:
            "Monthly limit reached. Free plan includes 30 meetings per month. Please upgrade to Premium for unlimited meetings!",
          limitReached: true,
          monthlyCount,
          limit: 30,
        });
...
export const getMeetingStats = async (req, res) => {
    ...
    const monthlyLimit = plan === "premium" ? 30;
